...
struct {
	...
	//page nume of the head, null if empty
	size_t base_num;
	rte_spinlock_t lock;
...
void ssd_ht_init(void) {
    ...
    ssd_free_pages.cur_page = get_new_page(1);


    page_buf_size = rte_lcore_count() * PAGES_PER_CORE;
		...
		while(size > 0)
		{
			if(rest == 0)
			{
				    ...
				    {	
				    	...
				    	ssd_free_pages.cur_page = get_new_page();			    	
				    }
...
void* get_new_page()
{
	...
	RTE_LOCK(&ssd_free_pages.lock);

		num = ssd_free_pages.page_num++;

	RTE_UNLOCK(&ssd_free_pages.lock);


	RTE_LOCK(&page_buffer.lock);

	    while(size >= PAGE_BUF_SIZE/2)
	    {
	    	struct page* p = &(page_buffer.pages[page_buffer.head]);
	    	bool gotLock = RTE_TRYLOCK(&(p.lock));
	    	if(gotLock){
	    		if(can_write_out(p)) {
		    		page_buffer.head = page_buffer.head >= PAGE_BUF_SIZE - 1 : 0 ? page_buffer.head + 1;
		    		page_buffer.base_num = get_page_num(&page_buffer.pages[page_buffer.head]);
		    		//once popped off page, release page buffer lock
		    		RTE_UNLOCK(&page_buffer.lock);
	    		}
...
size_t get_page_num(struct page* p)
{
